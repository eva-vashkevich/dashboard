...
describe('component: rke2', () => {
  ...
  */
  ...
  describe('kubernetes version resolution', () => {
    ...
    // versionOptions/selectedVersion/chartVersions/fetchRke2Versions now live in
    // shell/composables/useKubernetesVersions.ts - see useKubernetesVersions.test.ts for their coverage.
    // defaultVersion and cloudProviderOptions stay here as thin/local Options API computed.
  ...
  // initRegistry/setRegistryConfig/updateConfigs/toggleCustomRegistry/handleRegistryHostChanged/
  // handleRegistrySecretChanged now live in shell/composables/useRegistryConfig.ts - see
  // useRegistryConfig.test.ts.

  // getChartValue/initAddons/chartVersionKey/showAddons/refreshComponentWithYamls/updateValues/
  // syncChartValues/initYamlEditor/applyChartValues now live in
  // shell/composables/useChartAddons.ts - see useChartAddons.test.ts. (Their rke2.vue methods are
  // now thin wrappers forwarding to the real, addonVersions-driven chartVersionKey rather than a
  // mockable `this.chartVersionKey`, so the old characterization tests here no longer apply.)
