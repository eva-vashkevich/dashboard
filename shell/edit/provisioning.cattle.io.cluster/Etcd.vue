...
export default {
  ...
  computed: {
    ...
    argsEtcdExposeMetrics() {
      return !!this.selectedVersion?.serverArgs && !!this.selectedVersion?.serverArgs['etcd-expose-metrics'];
    },
    configEtcdExposeMetrics() {
      return !!this.value.spec.rkeConfig.machineGlobalConfig && !!this.value.spec.rkeConfig.machineGlobalConfig['etcd-expose-metrics'];
    },
